 ...
 * Displays and manages a single scene
 */
...
const SceneView: React.FC<SceneViewProps> = ({ contentId, sectionNumber, sceneNumber }) => {
  const { getSceneById, updateScene, loading, error } = useScenes();
  ...
  useEffect(() => {
    ...
    if (scene) {
      ...
      setEditedCharacters(scene.characters?.join(', ') || '');
  ...
  const loadScene = async (): Promise<void> => {
    try {
      const response = await getSceneById(contentId, sectionNumber, sceneNumber);
      if (response) {
        setScene(response);
      }
    } catch (error) {
  ...
  const handleSaveScene = async (): Promise<void> => {
    ...
    try {
      ...
      await updateScene(contentId, sectionNumber, sceneNumber, updateData);
      await loadScene();
      setIsEditing(false);
    } catch (error) {
